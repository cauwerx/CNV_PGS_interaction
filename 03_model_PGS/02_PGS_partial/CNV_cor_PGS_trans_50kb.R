# Correlation between PGS_trans [50kb] and CNV carrier status for 119 CNV-trait pairs
...
# PGS data for all individuals (PGS_trans ± 50kb around CNV region; filtered for testing samples IID) 
# File with sample identifier (IID) as first column, then one column per phenotype, containing PGS values
pgs <- as.data.frame(fread("CNV_PGS/data/PGS_continuous_out_50kb.txt.gz"))
...
fwrite(df, "CNV_PGS/data/model_PGS/CNV_cor_PGS_trans_50kb_.txt", col.names = T, row.names = F, quote = F, sep = "\t")
