# Correlation between PGS_trans [100kb] and CNV carrier status for 119 CNV-trait pairs
...
# PGS data for all individuals (PGS_trans ± 100kb around CNV region; filtered for testing samples IID) 
# File with sample identifier (IID) as first column, then one column per phenotype, containing PGS values
pgs <- as.data.frame(fread("CNV_PGS/data/PGS_continuous_out_100kb.txt.gz"))
...
for (i in 1:nrow(df)) {
  ...
  df_temp <- pgs[, names(pgs) %in% c("IID", paste0(p, "_chr", chr, "_", start-100000, "_", end+100000))]
...
fwrite(df, "CNV_PGS/data/model_PGS/CNV_cor_PGS_trans_100kb.txt", col.names = T, row.names = F, quote = F, sep = "\t")
