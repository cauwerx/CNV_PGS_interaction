# Correlation between PGS_trans [250kb] and CNV carrier status for 119 CNV-trait pairs
...
# PGS data for all individuals (PGS_trans ± 250kb around CNV region; filtered for testing samples IID) 
# File with sample identifier (IID) as first column, then one column per phenotype, containing PGS values
pgs <- as.data.frame(fread("CNV_PGS/data/PGS_continuous_out_250kb.txt.gz"))
...
for (i in 1:nrow(df)) {
  ...
  df_temp <- pgs[, names(pgs) %in% c("IID", paste0(p, "_chr", chr, "_", start-250000, "_", end+250000))]
...
fwrite(df, "CNV_PGS/data/model_PGS/CNV_cor_PGS_trans_250kb.txt", col.names = T, row.names = F, quote = F, sep = "\t")
