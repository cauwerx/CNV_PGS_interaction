...
for (i in 1:trials){
  ...
  for (j in 1:pairs_n){
    # 1) Simulate PGS (normal) and CNV (binomial/bernoulli)
    pheno_name <- param_df[j, 'phenotype']
    pheno_sd <- param_df[j, 'pheno_sd']
    sample_n <- param_df[j, 'n']
    carriers_n <- param_df[j, 'CNV_n']
    PGS_mean <- param_df[j, 'PGS_mean']
    PGS_sd <- param_df[j, 'PGS_sd']
    ...
    # 2) Simulate phenotypes, with a and b taken from real values
    pheno <- a*CNV + b*PGS + e
    
    # 3) Get CNV -> PGS and CNV -> trait effects through linear regression
    model_1 <- lm(pheno~CNV)
    model_2 <- lm(PGS~CNV)
    c1_list[j] <- coef(model_1)[["CNV"]]
    c2_list[j] <- coef(model_2)[["CNV"]]
  }
  # 4) Assess the ratio showing sign agreement across all 119 pairs
