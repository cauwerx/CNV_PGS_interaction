# Get the Pearson correlation p-value for all trait-CNV_encoding combinations
library(data.table)
...
################################################################################
# STEP 2: Create trait-CNV matrix of Pearson correlation p-values
################################################################################

p_m <- matrix(NA_real_, nrow=trait_n, ncol=cnv_n)
rownames(p_m) <- unique_traits
colnames(p_m) <- unique_cnvs

for (trait in unique_traits){
  for (cnv in unique_cnvs){
    p <- cor.test(geno_pheno[[trait]], geno_pheno[[cnv]], use = "complete.obs")$p.value
    p_m[trait, cnv] <- p
    cat(trait, cnv, p, '\n')
  }
}

fwrite(as.data.table(p_m, keep.rownames = "trait"), "07_sensitivity_analyses/01.trait_cnv.corr_pval.csv")
