# Model adjusted phenotypes according to CNV status, PGS, and their interaction for 119 CNV-trait pairs
...
for (i in 1:nrow(df)) {
  ...
  fit <- lm(PHENO ~ CNV + PGS + CNV:PGS, data = df_temp)
  ...
  df[i, "EFFECT_CNVxPGS"] <- summary(fit)$coefficients[4,1]
  df[i, "SE_CNVxPGS"] <- summary(fit)$coefficients[4,2]
  df[i, "P_CNVxPGS"] <- summary(fit)$coefficients[4,4]
...
fwrite(df, "CNV_PGS/data/model_phenotype/CNV_PGS_interaction_model.txt", col.names = T, row.names = F, quote = F, sep = "\t")