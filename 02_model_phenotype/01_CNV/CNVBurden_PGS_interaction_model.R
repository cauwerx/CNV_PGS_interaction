# Model adjusted phenotypes according to CNV burden, PGS, and their interaction for 43 assessed phenotypes
...
del_burden <- as.data.frame(fread("CNV_PGS/data/CNV_burden/DEL_burden.txt.gz", select = c(1,10), col.names = c("IID", "DEL_GENES")))
...
for (i in 1:nrow(df)) {
  ...
  fit_CNV <- lm(PHENO ~ CNV_GENES + PGS + CNV_GENES:PGS, data = df_temp)
  ...
  fit_DEL <- lm(PHENO ~ DEL_GENES + PGS + DEL_GENES:PGS, data = df_temp)
  ...
  fit_DUP <- lm(PHENO ~ DUP_GENES + PGS + DUP_GENES:PGS, data = df_temp)
...
fwrite(df, "CNV_PGS/data/model_phenotype/CNVBurden_PGS_interaction_model.txt", col.names = T, row.names = F, quote = F, sep = "\t")
