...
fwrite(pheno_int_all, paste0(OUT_DIR, "/pheno_continuous_WB_INT_All.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_int_M, paste0(OUT_DIR, "/pheno_continuous_WB_INT_M.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_int_F, paste0(OUT_DIR, "/pheno_continuous_WB_INT_F.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
for (p in 2:ncol(pheno_int_all)) {
        ...
        print(paste0("Covariates - All, pheno: ", pheno))
...
fwrite(pheno_int_cor_all, paste0(OUT_DIR, "/pheno_continuous_WB_INT_age_age2_sex_batch_PCs_All.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_int_cor_M, paste0(OUT_DIR, "/pheno_continuous_WB_INT_age_age2_batch_PCs_M.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_int_cor_F, paste0(OUT_DIR, "/pheno_continuous_WB_INT_age_age2_batch_PCs_F.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)

###########################################################################################################
# STEP 4: Correct for covariates on non-INT data
############################################################################################################
...
for (p in 2:ncol(pheno_all)) {
        ...
        temp <- left_join(pheno_all[, c(1,p)], cov_all, by = "IID")
	    colnames(temp)[2] <- "pheno"
        # Regress out covariates
        print(temp[1, c(1,ncol(temp))])
	    pheno_cor_all[, pheno] <- residuals(lm(pheno ~ . , data = temp[, -c(1)], na.action = na.exclude))
}; rm (p, pheno, temp)

fwrite(pheno_cor_all, paste0(OUT_DIR, "/pheno_continuous_WB_age_age2_sex_batch_PCs_All.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
for (p in 2:ncol(pheno_M)) {
        ...
        # Create a temporary dataframe
	    temp <- left_join(pheno_M[, c(1,p)], cov_M, by = "IID")
        colnames(temp)[2] <- "pheno"
        # Regress out covariates
        pheno_cor_M[, pheno] <- residuals(lm(pheno ~ . , data = temp[, -c(1)], na.action = na.exclude))
}; rm (p, pheno, temp)

fwrite(pheno_cor_M, paste0(OUT_DIR, "/pheno_continuous_WB_age_age2_batch_PCs_M.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
for (p in 2:ncol(pheno_F)) {
        ...
        # Create a temporary dataframe
        temp <- left_join(pheno_F[, c(1,p)], cov_F, by = "IID")
	    colnames(temp)[2] <- "pheno"
        # Regress out covariates
        pheno_cor_F[, pheno] <- residuals(lm(pheno ~ . , data = temp[, -c(1)], na.action = na.exclude))
}; rm (p, pheno, temp)

fwrite(pheno_cor_F, paste0(OUT_DIR, "/pheno_continuous_WB_age_age2_batch_PCs_F.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
