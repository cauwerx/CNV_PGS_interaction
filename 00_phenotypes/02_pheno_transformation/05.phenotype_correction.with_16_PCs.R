...
## Read in covariates
# Read up to PC16
PC_n <- 16+6 # PCs start from col 7
cov <- as.data.frame(fread(paste0(DIR, "/data/covariates.txt"), header = T, select = c(1:4, 6:PC_n)))
colnames(cov)[1] <- "IID"
...
fwrite(pheno_int_cor_all, paste0(OUT_DIR, "/pheno_continuous_test_INT_age_age2_sex_batch_PCs_All.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_int_cor_M, paste0(OUT_DIR, "/pheno_continuous_test_INT_age_age2_batch_PCs_M.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_int_cor_F, paste0(OUT_DIR, "/pheno_continuous_test_INT_age_age2_batch_PCs_F.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_cor_all, paste0(OUT_DIR, "/pheno_continuous_test_age_age2_sex_batch_PCs_All.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_cor_M, paste0(OUT_DIR, "/pheno_continuous_test_age_age2_batch_PCs_M.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(pheno_cor_F, paste0(OUT_DIR, "/pheno_continuous_test_age_age2_batch_PCs_F.tsv"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
